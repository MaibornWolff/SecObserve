...
def _check_notifications(notification_ids: list[int]) -> QuerySet[Notification]:
    ...
    if not user.is_superuser:
        for notification in notifications:
            if not notification.product or not user_has_permission(notification.product, Permissions.Product_Delete):
                raise ValidationError("User does not have permission to delete some notifications")
