...
def _calculate_active_config_security_gate(product: Product) -> bool:
    ...
    if (
        ...
        or product.open_low_observation_count > config.SECURITY_GATE_THRESHOLD_LOW
        or product.open_none_observation_count > config.SECURITY_GATE_THRESHOLD_NONE
        or product.open_unkown_observation_count > config.SECURITY_GATE_THRESHOLD_UNKOWN
    ):
        new_security_gate_passed = False
