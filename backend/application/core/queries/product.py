...
def get_products(
    ...
    if not is_product_group and with_annotations:
        ...
        subquery_license_forbidden = _get_license_subquery(
        ...
        subquery_license_unknown = _get_license_subquery(
            License_Policy_Evaluation_Result.RESULT_UNKNOWN
        )
        subquery_license_allowed = _get_license_subquery(
            License_Policy_Evaluation_Result.RESULT_ALLOWED
        )
        subquery_license_ignored = _get_license_subquery(
        ...
        products = products.annotate(
            ...
            review_required_licenses_count=Coalesce(
                subquery_license_review_required, 0
            ),
