...
def get_license_group_member(license_group: License_Group, user: User) -> Optional[License_Group_Member]:
    try:
...
def get_license_group_members() -> QuerySet[License_Group_Member]:
    ...
    return license_group_members.filter(license_group__in=license_groups, user__in=users)
