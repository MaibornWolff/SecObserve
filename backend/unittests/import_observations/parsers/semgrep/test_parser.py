...
class TestSemgrepParser(TestCase):
    ...
    def test_wrong_format(self):
        with open(path.dirname(__file__) + "/files/wrong_format.json") as testfile:
            ...
            self.assertFalse(check)
            self.assertEqual(1, len(messages))
            self.assertEqual("File is not a Semgrep format, version is missing", messages[0])
            self.assertFalse(data)
    ...
    def test_multiple_observations(self):
        with open(
            ...
            self.assertEqual("python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query", observation.title)
...
**Vulnerability Class:** SQL Injection"""
            ...
            self.assertEqual(
                "application/management/commands/command.py", observation.origin_source_file
            )
            ...
            self.assertEqual("https://semgrep.dev/r/python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query", observation.unsaved_references[0])
            self.assertEqual("https://docs.sqlalchemy.org/en/14/core/tutorial.html#using-textual-sql", observation.unsaved_references[1])
            ...
            self.assertEqual("python.lang.security.insecure-hash-algorithms-md5.insecure-hash-algorithm-md5", observation.title)
...
**Vulnerability Classes:** Cryptographic Issues, Other Issues"""
            ...
            self.assertEqual("python.lang.security.use-defusedcsv.use-defusedcsv", observation.title)
            recommendation = """```
